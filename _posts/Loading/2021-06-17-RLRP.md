...
如图？？和图？？数据是（x，10^6, 3）下的标准差和P。可以看出在RLRP-pa分布效果最佳，数据分布最为公平。标准差不随数据节点增加而增加，P值略有增加，但也是最小的。crush，random slicing和kinesis三种基于伪哈希的放置表现也很好，其中kinesis波动较大，主要。。。Consistent Hashing表现一般，而DMORP表现最为糟糕，如图右边所示，远远差于其他算法。

图？表现了在不同对象数下的公平性表现，可以看到，RLRP-pa非常稳定P在1%左右，在所有情况表现最佳且波动很小。而crush，random slicing和kinesis四种基于hash的算法，在小数据样本下表现的很差，P值为25%~30%，随着数据量增加，表现和RLRP-pa类似。而Consistent Hashingp值在5%~20%，DMORP依然最差，p值高于50%。图？表现了不同
