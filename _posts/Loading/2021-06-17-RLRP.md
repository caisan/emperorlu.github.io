...
![image-20210820142222346](..\..\photos\image-20210820142222346.png)
...
在本节中，我们将描述一种新的基于RL的数据分布方案，for现在分布式存储系统。该策略试图通过结合基于表和伪随机哈希策略的经验教训，克服当前数据分布策略的局限性，解决RL模型的挑战，并且为复杂的异构系统提供优化性能。



<img src="..\..\photos\RL\RLRP.png" alt="RL" style="zoom: 18%;" />

如图？？所示，我们基于RL模型，定义了Environment和Agent基本元素，整个



Environment：如图？？，对于我们不直接映射objects到DNs，而是先通过Hash函数将objects分组到Virtual Nodes，这是管理数据的一种基本方式，就像Ceph的PG（），，，，。我们的算法将VNs映射到DNs。在每一步中，每个VN分配到DN将由算法决定，算法会考虑DNs节点的状态。算法得出action即映射关系将，有一个映射表存储。每次读将通过查表找VN所在位置。由于经过hash分组，映射表不会像集中式那样随着objects增长而线性增长。

Common interface主要是RLRP和Environment交互的接口，包括Metrics Collector和Action Controller两部分，Metrics Collector负责从环境中获取DNs节点的相关属性，

RL Agent是RL模型的主体，分为Placement Agent和Migration Agent，分别为分布和迁移做出决策，
