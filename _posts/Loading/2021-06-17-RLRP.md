...
在分布式系统中，数据分布算法对于性能和可靠性是非常重要的，尤其是面对  数据量



面临着在数十甚至是数百数千的存储设备之间分发PB这个数量级别数据的艰巨任务

1. 均匀性
2. 可扩展性
3. 可靠性

这样的系统必须能够均匀的分配数据和工作负载，以获取对可用资源的高效使用，和系统性能的最大化, 同时要便于系统的扩展以及对硬件故障的管理



- （有很多副本放置算法，**存在问题**）

传统的数据分布算法可以分成table-based和random-based，或者中心化或者去中心化的设计。典型的

随机分布/去中心化的

数据量增加，

动态适应不断变化的工作负载，这些工作负载通常包括数据访问偏差。

此外，底层物理存储节点可能是异构的，并且不会将它们的性能能力暴露给更高级别的数据放置层。



- （**提出基于强化学习的副本放置算法**）

不少研究表明，动态的副本放置是一个组合优化问题，而RL是解决组合优化问题
