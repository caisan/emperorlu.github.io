...
- 模型优化
  ...
  - **问题2：OSD、PG增加训练困难**
    ...
    - **OSD增加？**
    - 不考虑PG差异情况下
      - 设置一个训练区间，比如设置训练区间为100，测试1000pg时可以分成10个100的训练区间进行
      - [100,121,101]  == [0,21,1]，每次将状态组所有元素减去最小的值，得到当前状态
  ...
  - 演示效果
    ...
    - 10000 pg，拆分成100个训练区间进行测试

      - cost time:  0.35152244567871094 s

      <img src="..\..\photos\RL\test3_osd.png" alt="test3_osd" style="zoom:67%;" /><img src="..\..\photos\RL\test3_st.png" alt="test3_st" style="zoom: 67%;" />

      - 三副本 cost time:  12.70314908027649 s

      <img src="..\..\photos\RL\test4_osd.png" alt="test4_osd" style="zoom:67%;" /><img src="..\..\photos\RL\image-20210602195301847.png" alt="image-20210602195301847" style="zoom: 50%;" />
...
<table>
    ...
    <tr>
        <td width="23%" rowspan="2">一、基本功能，简单Demo</td>    
        <td width="14%" >分布均匀</td>  
        <td width="14%" >最小化迁移</td> 
        <td width="14%" >高效查询</td> 
        <td width="14%" >主副本</td> 
        <td >模型简单训练快速</td> 
    </tr>
    ...
    <tr>
        <td >优，DDPG</td>  
        <td ><b>差</b>，Attention模型</td> 
        <td >优，训练区间</td> 
        <td >优，模型保存与参数固定</td> 
        <td ><b>中</b>，原状态下重新训练</td>  
    </tr>
    <tr>
        <td rowspan="2">三、异构环境，动态适应</td>    
        <td >不同权重</td>  
        <td >不同介质</td> 
        <td >不同负载环境</td> 
        <td >对象大小、ID等特征</td> 
...
- TC‘19, IEEE Transactions on Computers，A类期刊
