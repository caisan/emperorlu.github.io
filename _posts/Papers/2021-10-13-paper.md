...
- KV读优化，根据之前分析，可以基于几个方向出发
  ...
  - Cache：提高缓存命中率。由于在 LSM-tree 中数据从内存写入磁盘和磁盘中不同层间的数据合并操作，造成缓存失效问题，降低了缓存命中率。可以考虑设计**相关算法（相关索引、机器学习等）**根据工作负载和 LSM-tree 内部的操作特点，预测未来可能访问的数据并将其预取到缓存中，从而降低缓存失效问题
    ...
    - **主题1**：高效Cache for LSM-tree-based KV store
  ...
  - LSM-tree 范围查询优化。目前, 大多数查询优化方法主要是针对 LSM-tree 点查询进行优化，对范围查询的优化较少，而 LSM-tree 对范围查询更不友好。使用filter来LSM优化范围查询无法避免对数据的探查与探查结果的合并开销，并且不适合长范围查询场景。REMIX 对数据建立全局索引能够避免这些开销，但是插入新数据时索引重构开销较大。能否考虑将**两者优势结合**在一起?
    ...
    - **主题2**：高效范围查询 for LSM-tree-based KV store
      - 拟定优化方向：1. 支持范围查询的 learned index filter ；2. 大range范围：索引优化；3. 结合负载
...
1 . 任国光硕1904 班组织委员 (2019.09 - 至今) ;\n2 . 任信息存储与光显示功能实验室学生第三党支部组织委员 (2020.09 - 至今) ；\n3 . 获2020年度 - 优秀研究生干部；\n4 . 投稿POEM - ISST，poster已收录；\n5 . 以第一作者投稿 DATE 2022 学术会议，正在等待结果；\n6 . 积极参与实验室的项目、专利撰写及基金撰写;\n7 . 负责管理实验室的各项活动经费，并在2020 年底被实验室评为“服务之星”；\n8 . 加权成绩15/39 ，达到要求。

### 基本战略：稳中求险

- 经过之前”**三步走**“战略，到了工作结构调整时期，采取”**稳中求险**“作为新的基本战略

- **稳：**以KV读优化为当前主要目标，思考一些filter/cache/索引优化方案。进行一些测试，寻找不同场景/负载下的瓶颈 —> 尝试阶段
- **险1：**Ceph测试，性能优化，寻找新瓶颈  —>  看项目发展
- **险2：**了解强化学习和DI-engine，寻找问题和场景 —>  学习阶段
- 未知因素：trace收集？后续项目？
- **NEXT**：“稳”中出1篇文章；“险1”和“险2”出1篇文章
